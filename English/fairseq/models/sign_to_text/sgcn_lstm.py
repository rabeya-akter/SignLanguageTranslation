...
class Sgcn_Lstm(nn.Module):
    def __init__(self, train_x=None, train_y=None, AD=None, AD2=None, lr=0.0001, epoach=200, batch_size=10):
        ...
        self.sgcn_4=sgcn(graph.AD, graph.AD2, graph.bias_mat_1, graph.bias_mat_2)
        self.sgcn_5=sgcn(graph.AD, graph.AD2, graph.bias_mat_1, graph.bias_mat_2)

    def lstm(self,x):
        x = x.view(x.shape[0], -1, x.shape[1] * x.shape[2])
        rec,_ = nn.LSTM(input_size=x.shape[2], hidden_size=128, num_layers=1, batch_first=True, dropout=0.25, bidirectional=False).to('cuda:0')(x)
        rec1,_ = nn.LSTM(input_size=rec.shape[-1], hidden_size=256, num_layers=1, batch_first=True, dropout=0.25, bidirectional=False).to('cuda:0')(rec)
        rec2,_ = nn.LSTM(input_size=rec1.shape[-1], hidden_size=256, num_layers=1, batch_first=True, dropout=0.25, bidirectional=False).to('cuda:0')(rec1)
        """
